...
retFut1(end, :, end) = 0; % no data available for last day

% cut off out-of-range days
prevstartdate=tday(startdateIdx-1);
% tdayFut=tday(startdateIdx:enddateIdx);
tday=tday(startdateIdx-1:enddateIdx);
retFut1([1:startdateIdx-2 enddateIdx+1:size(retFut1, 1)], :, :)=[];
my_cls([1:startdateIdx-2  enddateIdx+1:size(my_cls, 1)], :, :)=[];

retFut1(~isfinite(retFut1))=0;
...
if (~isAdjustSharesDaily)
    ...
    for d=1:size(positionTable, 1)
        if (d>1)
            positionTableAdj(d, :, 1)=positionTableAdj(d-1, :, end).*(1+retFut1(d-1, :, end));
        ...
        for h=2:size(positionTable, 3)
            positionTableAdj(d, :, h)=positionTableAdj(d, :, h-1).*(1+retFut1(d, :, h-1));
...
% for 0 cost
% pnl = smartsum(smartsum(positionTable(:, :, 1:end-1).* retFut1(:, :,
% 1:end-1), 3), 2);

retFut1_=retFut1;
retFut1_(isnan(retFut1_))=0;
pnlstk=zeros(size(positionTable(:, :, 1)));

for h=1:size(positionTable, 3)-1
    pnlstk=pnlstk+positionTable(:, :, h).* retFut1_(:, :, h);
end
clear retFut1_;
...
ret=backshift(1, squeeze(retFut1(:, :, end)));
ret(1, :)=0;
pnl = pnl + smartsum(positionTable0(:, :, end).*ret, 2);
clear ret;
...
for s=1:size(positionTable, 2)
    pnld_s=positionTable(:, s, 1:end-1).*retFut1(:, s, 1:end-1) - costSlip(:, s, 2:end) - costTrans(:, s,  2:end);
...
pnld(:, end+1)=smartsum(positionTable0(:, :, end).*ret - costSlip(:, :, 1) - costTrans(:, :, 1), 2);
