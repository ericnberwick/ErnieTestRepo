...
% 
% COLHEADER = MYTEXTREAD(FILENAME)
%
% Reads the column headers of files written out by cell2txt.
%
% [ch,field1,field2] = mytextread(filename,{'field1'; 'field2'});
...
for k=1:NCol,
  h1 = 0;
  for h=1:NColToRead,
    if isempty(strmatch([ColToRead{h,1} ':'],ColHeader{k}))==0,
      h1 = h;
    end,
  end,
  if h1,
...
eval(['[' OutStr '] = textread(''' filename ''',''' FmtStr ''',''delimiter'',''\b\t'',''headerlines'',1);']); % do not allow , to be delimiter

for h=1:NColToRead,
  eval(['varargout{h} = ' ColToRead{h,1} ';']);
end,
